...
#' @param content character. Email content.
...
#' @param format character. Type of format desired. Options are text or html.
#' @export
send_email <- function(from, to, subject, content, attach = NULL, tag = NULL, format = c("text", "html")) {
  if (length(from) == 1) {
    from <- paste0(from, "@", mailgun_domain_name())
  } else {
    from <- paste0(from[1], " <", from[2], "@", mailgun_domain_name(), ">")
  }
  format_type <- match.arg(format)

  if(format_type != "text") {
    payload <- list(from = from, subject = subject, html = content)
  } else {
    payload <- list(from = from, subject = subject, text = content)
  }
